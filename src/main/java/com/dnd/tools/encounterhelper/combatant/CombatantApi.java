...
public class CombatantApi {
  ...
  public List<Combatant> newRound(@RequestBody Map<Long, Integer> playerIdAndInitiativeRoll) {
    ...
    combatants.stream()
        .filter(combatant -> !combatant.isNpc())
        .forEach(
            player -> player.setCurrentInitiative(playerIdAndInitiativeRoll.get(player.getId())));
    ...
    // Reduce any conditions with expiry counts
    combatants.forEach(combatant -> {
      List<ConditionTracker> conditionTracker = combatant.getConditionTracker();
      if (conditionTracker != null) {
        conditionTracker.forEach(condition -> {
          if (condition.getRemainingRounds() != null) {
            condition.setRemainingRounds(condition.getRemainingRounds() - 1);
          }
        });
        conditionTracker.removeIf(condition -> {
          Integer remainingRounds = condition.getRemainingRounds();
          if (remainingRounds != null && remainingRounds == 0) {
            return false;
          }
          return true;
        });
        combatant.setConditionTracker(conditionTracker);
      }
    });
